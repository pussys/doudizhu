            ...
            )
        {
            SingleExecute.Instance.Execute(
                delegate ()
                {
                    ...
                    //判断一下 这个账号以前有没有角色
                    if (userCache.IsExist( client, accountId))
                    {
                        client.Send(OpCode.USER, UserCode.CREATE_SRES, -2);//"已经有角色 不能重复创建"
                        return;
                    }
                    //没有问题 才可以创建
                    userCache.Create( client, name, accountId);
        ...
        private void getInfo(ClientPeer client)
        {
            SingleExecute.Instance.Execute(
                  delegate ()
                  {
                      ...
                      int accountId = accountCache.GetId(client);
                      if (userCache.IsExist( client, accountId) == false)
                      {
                      ...
                      //给客户端发送自己的角色信息
                      UserModel model = userCache.GetModelByAccountId( client, accountId);
        ...
        private void online(ClientPeer client)
        {
            SingleExecute.Instance.Execute(
                   delegate ()
                   {
                       ...
                       //上线成功了
                       int userId = userCache.GetId( client, accountId);
