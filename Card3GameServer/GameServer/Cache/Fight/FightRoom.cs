...
namespace GameServer.Cache.Fight
{
    ...
    public class FightRoom
    {
        ...
        public bool DeadCard(int type, int weight, int length, int userId, List<CardDto> cardList)
        {
            ...
            {
                //特殊的类型 ：顺子  还要进行长度的限制
                if (type == CardType.STRAIGHT || type == CardType.DOUBLE_STRAIGHT || type == CardType.TRIPLE_STRAIGHT||type==CardType.FEIJI||type==CardType.SIDAIER
                    || type == CardType.SIDAIDUI)
                {
            ...
            //普通的炸弹 可以管 任何不是炸弹的牌 
            else if (type == CardType.BOOM && roundModel.LastCardType != CardType.JOKER_BOOM&&roundModel.LastCardType!=CardType.BOOM)
            {
                //满足出牌条件
                canDeal = true;
        ...
        public int GetFirstUId()
        {
            ...
            return PlayerList[rd.Next(0,3)].UserId;
        }
